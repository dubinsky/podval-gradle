...
| `org.podval.conventions` | `build.gradle` | Applies `io.github.ben-manes.versions` and the shared `rejectVersionIf` (non-stable + padded numeric `0.017`). If `java` is present (covers `java-library` / `scala`): Java 25 toolchain. If `scala` is present: `-release:25`, `-new-syntax`, `-feature`, `-language:strictEquality`, `-source:future`. Does **not** skip `compileJava`. |
| `org.podval.conventions.publish` | `build.gradle` (libraries only) | `javadocJar` from ScalaDoc **before** `withJavadocJar()`; `withSourcesJar()`; jar `duplicatesStrategy = EXCLUDE` and Implementation-* manifest; Maven `library` publication POM (SCM, issues, CI, Apache-2.0, developer) and in-memory signing from `signingKey` / `signingPassword`. |
